...
#python writes back to the db the count for each temperature
count_pipeline = [
	{"$unwind":"$temp"},
...
#python writes back to the db the avg for each timestamp
avg_pipeline = [
	{"$unwind":"$timestamp"},
	{"$group": {"_id":"$timestamp", "avg": {"$avg": 1}}},
	{"$sort": SON([("avg", -1), ("_id", -1)])}
	]

pprint.pprint(list(db.scratch_data.aggregate(avg_pipeline)))